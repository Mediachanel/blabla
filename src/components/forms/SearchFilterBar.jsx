    ...
    <section className="rounded-lg border border-slate-200 bg-white p-3 sm:p-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <label className="relative block min-w-0 flex-1">
          <span className="section-label mb-2 block">Pencarian Pegawai</span>
          <Search className="pointer-events-none absolute left-3 top-[2.35rem] h-4 w-4 text-slate-400" aria-hidden="true" />
          <input className="input h-12 pl-9" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Cari nama, NIP, jabatan, atau UKPD" />
        </label>
        <div className="grid gap-3 sm:grid-cols-2 lg:flex lg:flex-wrap lg:items-end">
          {filters.map((filter) => (
            <label key={filter.name} className="min-w-0 lg:min-w-44">
              <span className="section-label mb-2 block">{filter.label}</span>
              <select className="input h-12" value={filter.value} onChange={(event) => filter.onChange(event.target.value)}>
