...
export default function Sidebar({ user, collapsed, onToggle }) {
  ...
  return (
    <aside className={`hidden border-r border-[#e9edf3] bg-white shadow-[4px_0_18px_rgba(15,23,42,0.04)] transition-[width] duration-200 lg:fixed lg:inset-y-0 lg:flex lg:flex-col ${collapsed ? "lg:w-16" : "lg:w-56"}`}>
      <header className={`flex h-16 items-center gap-3 px-4 ${collapsed ? "justify-center px-2" : ""}`}>
        ...
        {!collapsed ? (
          <div className="min-w-0">
            <p className="text-sm font-bold leading-tight text-slate-900">SI-DATA</p>
            <p className="truncate text-xs font-medium text-slate-500">Dinkes DKI Jakarta</p>
          </div>
        ) : null}
      </header>
      <section className={`flex items-center gap-3 border-y border-[#e9edf3] px-4 py-3 ${collapsed ? "justify-center px-2" : ""}`}>
        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-slate-100 text-xs font-medium text-slate-500 shadow-sm ring-1 ring-slate-200">
          {getInitials(user)}
        </span>
        {!collapsed ? (
          <div className="min-w-0">
      ...
      <footer className="border-t border-[#e9edf3] p-2">
