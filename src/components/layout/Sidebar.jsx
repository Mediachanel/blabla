...
export default function Sidebar({ user, collapsed, onToggle }) {
  ...
  return (
    <aside className={`hidden border-r border-[#e9edf3] bg-white shadow-[4px_0_18px_rgba(15,23,42,0.04)] transition-[width] duration-200 lg:fixed lg:inset-y-0 lg:flex lg:flex-col ${collapsed ? "lg:w-14" : "lg:w-[188px]"}`}>
      <header className={`flex h-14 items-center gap-2.5 px-3 ${collapsed ? "justify-center px-2" : ""}`}>
      ...
      <section className={`flex items-center gap-2.5 border-y border-[#e9edf3] px-3 py-3 ${collapsed ? "justify-center px-2" : ""}`}>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-2 py-4" aria-label="Menu utama">
        {menu.map((item) => (
